...
<main class="container mx-auto mt-12 mb-20 px-6 max-w-2xl">
    <div class="mb-8 text-center">
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Transaction</h1>
        <p class="text-slate-500 mt-2">Update your transaction details</p>
    </div>
                ...
                <div class="relative">
                    ...
                    <select name="category_id" required
                        ...
                        <?php foreach ($categories as $category): ?>
                            <option value="<?= e($category['id']) ?>" <?= $transaction['category_id'] == $category['id'] ? 'selected' : '' ?>>
                                <?= e($category['name']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div class="space-y-2">
                ...
                <div class="flex gap-3">
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="transaction_type" value="expense" <?= ($transaction['transaction_type'] ?? 'expense') === 'expense' ? 'checked' : '' ?>
                        class="peer sr-only" />
                        <div
                    ...
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="transaction_type" value="income" <?= ($transaction['transaction_type'] ?? '') === 'income' ? 'checked' : '' ?>
                        class="peer sr-only" />
                        <div
